...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (savedGroups) {
      try {
        ...
        setActiveGroups(parsedGroups.filter((g: any) => g.activity !== "none").length)
      } catch (error) {
  ...
  const handleCreateGroup = (e: React.FormEvent) => {
    ...
    const newGroupData = {
      ...
      memberCount: newGroup.members ? newGroup.members.split(",").length + 1 : 1, // +1 for the creator
      activity: "high",
      balance: 0,
      balanceType: "neutral",
  ...
  const handleAddExpense = (expenseData: any) => {
    const newExpense = {
      ...
      paidBy: expenseData.paidBy === "you" ? "You" : expenseData.paidByPerson || "You",
    ...
    <div className="space-y-6">
          ...
          <Dialog open={openGroupDialog} onOpenChange={setOpenGroupDialog}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>Create a new group to track shared expenses</DialogDescription>
              </DialogHeader>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {activeGroups > 0 ? `${groups.length} total groups` : "No groups created yet"}
            </p>
          </CardContent>
        </Card>
      </div>
      <div className="grid gap-4 md:grid-cols-7">
        <Card className="md:col-span-4">
          <CardHeader className="flex flex-row items-center">
            <div className="grid gap-2">
              ...
              <CardDescription>Your recent expenses across all groups</CardDescription>
            </div>
            <Link href="/dashboard/expenses" className="ml-auto flex items-center gap-1 text-sm font-medium">
              View All
          ...
          <CardContent>
            {expenses.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <p className="text-sm text-muted-foreground">No expenses yet. Add your first expense to get started.</p>
                <Button className="mt-4" onClick={() => setOpenExpenseDialog(true)}>
                  <Plus className="mr-2 h-4 w-4" />
                  Add Expense
                </Button>
              </div>
        ...
        <Card className="md:col-span-3">
          ...
          <CardFooter>
            <Button className="w-full" variant="outline" disabled={expenses.length === 0}>
              Settle Up
            </Button>
          </CardFooter>
        </Card>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
        <Card className="lg:col-span-4">
          ...
          <CardContent>
            {groups.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <Button className="mt-4" onClick={() => setOpenGroupDialog(true)}>
                  <Plus className="mr-2 h-4 w-4" />
                  Create Group
                </Button>
              </div>
          ...
          <CardFooter>
            <Button asChild variant="ghost" className="w-full" disabled={groups.length === 0}>
        ...
        <Card className="lg:col-span-3">
          ...
          <CardContent>
            {categoryData.length > 0 ? (
                ...
                {categoryData.slice(0, 5).map((category, index) => (
                  <div key={category.name} className="space-y-2">
                    <div className="flex items-center">
                      ...
                      <div className="ml-auto text-sm">${category.amount.toFixed(2)}</div>
                    </div>
                    <Progress
                      value={Math.min(100, (category.amount / Math.max(...categoryData.map((c) => c.amount))) * 100)}
                      className="h-2"
                    />
                  </div>
                ))}
              ...
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <p className="text-sm text-muted-foreground">No spending data to display yet.</p>
              </div>
            )}
          </CardContent>
          <CardFooter>
            <Button asChild variant="ghost" className="w-full" disabled={expenses.length === 0}>
