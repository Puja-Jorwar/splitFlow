      ...
      const formattedExpenses = propExpenses.map((expense) => ({
        ...
        paidBy: {
          ...
          initials:
            expense.paidBy === "You"
              ? "YO"
              : expense.paidBy && typeof expense.paidBy === "string"
                ? expense.paidBy
                    .split(" ")
                    .map((n: string) => n[0])
                    .join("")
      ...
      {displayExpenses.map((expense) => (
        <div key={expense.id} className="flex items-center">
          <Avatar className="h-9 w-9">
            <AvatarImage
              src={expense.paidBy.avatar || "/placeholder.svg"}
          ...
          <div
            className={cn(
              "ml-auto font-medium",
              expense.type === "you_paid" ? "text-green-600" : "text-red-600",
            )}
          >
            {expense.type === "you_paid" ? "+" : "-"}$
